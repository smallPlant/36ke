"""36氪 PitchHub HTTP 客户端：Gateway API 与页面 HTML 抓取。"""
...
from kr36.core.config import DEFAULT_DELAY_MAX, DEFAULT_DELAY_MIN, GATEWAY_BASE, PITCHHUB_ORIGIN
...
class PitchHubClient:
    ...
    def gateway_post(self, path: str, param: dict[str, Any], *, referer: str) -> dict[str, Any]:
        """调用 PitchHub Gateway POST API，返回完整响应 JSON。"""
        self._throttle()
        payload = {
            "partner_id": "web",
    ...
    def parse_init_props(html: str) -> dict[str, Any] | None:
        """从页面 HTML 中提取 window.__INIT_PROPS__ JSON（括号深度匹配）。"""
